...
TEST(TestTokenize, TestWord) {
    ...
    UniString hi("Привет");
    base::GraphemTag t = GraphemTag::CAP_START | GraphemTag::CYRILLIC;
    std::vector<std::shared_ptr<base::Token>> result = tok.analyze(hi);
    EXPECT_EQ(result.size(),1);
    EXPECT_EQ(result[0]->getType(), base::TokenTypeTag::WORD);
    EXPECT_EQ(result[0]->toString(), hi);
    EXPECT_EQ(result[0]->getTag(), t);

    UniString ohi("ПрИвЕт");
    std::vector<std::shared_ptr<base::Token>> r = tok.analyze(ohi);
    EXPECT_EQ(r.size(), 1);
    GraphemTag ot = GraphemTag::MIXED;
    ot |= GraphemTag::CYRILLIC;
    EXPECT_EQ(r[0]->getType(), base::TokenTypeTag::WORD);
    EXPECT_EQ(r[0]->toString(), ohi);
    EXPECT_EQ(r[0]->getTag(), ot);

    UniString hi1("ПРИВЕТ");
    std::vector<std::shared_ptr<base::Token>> r1 = tok.analyze(hi1);
    EXPECT_EQ(r1.size(), 1);
    EXPECT_EQ(r1[0]->getType(), base::TokenTypeTag::WORD);
    ...
    UniString hi2("HeLLo");
    std::vector<std::shared_ptr<base::Token>> r2 = tok.analyze(hi2);
    EXPECT_EQ(r2.size(), 1);
    EXPECT_EQ(r2[0]->getType(), base::TokenTypeTag::WORD);
    ...
    std::vector<std::shared_ptr<base::Token>> r3 = tok.analyze(hi3);
    EXPECT_EQ(r3.size(), 1);
    EXPECT_EQ(r3[0]->getType(), base::TokenTypeTag::WORD);
    ...
    std::vector<std::shared_ptr<base::Token>> r4 = tok.analyze(hi4);
    EXPECT_EQ(r4.size(), 1);
    EXPECT_EQ(r4[0]->getType(), base::TokenTypeTag::WRNM);
...
TEST(TestTokenize, TestNumber) {
    ...
    std::vector<std::shared_ptr<base::Token>> r = tok.analyze(n);
    EXPECT_EQ(r.size(), 1);
...
TEST(TestTokenize, TestPunct){
    ...
    std::vector<std::shared_ptr<base::Token>> r = tok.analyze(p);
    EXPECT_EQ(r.size(), 1);
    ...
    std::vector<std::shared_ptr<base::Token>> r1 = tok.analyze(p1);
    EXPECT_EQ(r1.size(), 1);
    ...
    std::vector<std::shared_ptr<base::Token>> r3 = tok.analyze(p3);
    EXPECT_EQ(r3.size(), 2);
    ...
    std::vector<std::shared_ptr<base::Token>> r4 = tok.analyze(p4);
    EXPECT_EQ(r4.size(), 4);
...
TEST(TestTokenize, TestSeparator) {
    ...
    std::vector<std::shared_ptr<base::Token>> r = tok.analyze(s);
    EXPECT_EQ(r.size(), 1);
    ...
    std::vector<std::shared_ptr<base::Token>> r1 = tok.analyze(s1);
    EXPECT_EQ(r1.size(), 1);
    ...
    std::vector<std::shared_ptr<base::Token>> r2 = tok.analyze(s2);
    EXPECT_EQ(r2.size(), 1);
...
TEST(TestTokenize, TestSerious) {
    ...
    std::vector<std::shared_ptr<base::Token>> r2 = tok.analyze(p4);
    int counter = 0;
    for(int i = 0; i < r2.size(); ++i){
        if(r2[i]->getType() & base::TokenTypeTag::PNCT){
            std::shared_ptr<base::Token> pnct = r2[i];
            if (pnct->getTag() & base::GraphemTag::DOT){
                counter++;
            }
        }
    }
